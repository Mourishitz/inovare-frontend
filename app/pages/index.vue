<template>
  <div>
    ...
    <div class="mb-6">
      <UButton to="/signup" icon="i-heroicons-plus">
        Criar Novo Chá
      </UButton>
    </div>
    ...
    <div v-else class="space-y-6">
      ...
      >
        <template #header>
          <div class="flex justify-between items-start">
            <div>
              ...
              <p class="text-sm text-gray-500 mt-1">Anfitrião: {{ shower.host.username }}</p>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div class="space-y-4">
            <div>
              <h4 class="font-semibold text-gray-700 mb-2">Detalhes do Evento</h4>
              <div class="space-y-2 text-sm">
                ...
                <div class="flex items-start gap-2">
                  ...
                  <span class="font-medium">{{ formatDate(shower.shower_date) }}</span>
                </div>
                <div class="flex items-start gap-2">
                  <span class="text-gray-500 min-w-[120px]">Data do Casamento:</span>
                  <span class="font-medium">{{ formatDate(shower.wedding_date) }}</span>
                </div>
                <div class="flex items-start gap-2">
            ...
            <div v-if="shower.catalog.url">
              <UButton :to="shower.catalog.url" target="_blank" color="primary" block>
                Ver Catálogo
              </UButton>
            </div>
          </div>
          ...
          <div class="space-y-4">
            <div>
              ...
              <div class="space-y-2 text-sm">
                <div class="flex items-start gap-2">
                  ...
                  <span class="font-medium">{{ shower.preferences.style }}</span>
                </div>
                <div class="flex items-start gap-2">
                ...
                <div class="flex items-start gap-2">
                  ...
                  <span class="font-medium">{{ shower.preferences.preferredBra }}</span>
                </div>
                <div class="flex items-start gap-2">
                  <span class="text-gray-500 min-w-[120px]">Modelo:</span>
                  <span class="font-medium">{{ shower.preferences.preferredModel }}</span>
                </div>
                <div class="flex items-start gap-2">
                  <span class="text-gray-500 min-w-[120px]">Calcinha:</span>
                  <span class="font-medium">{{ shower.preferences.preferredPanties }}</span>
                </div>
              </div>
            </div>

            <div v-if="shower.preferences.favoriteColors.length > 0">
              <h5 class="text-sm font-semibold text-gray-700 mb-2">Cores Favoritas</h5>
              <div class="flex flex-wrap gap-1">
                <span
                  v-for="color in shower.preferences.favoriteColors"
                  :key="color"
                  :style="{ backgroundColor: getColorHex(color), color: getTextColor(color) }"
            ...
            <div v-if="shower.preferences.allowedModels.length > 0">
              <h5 class="text-sm font-semibold text-gray-700 mb-2">Modelos Permitidos</h5>
              <div class="flex flex-wrap gap-1">
                <UBadge
            ...
            <div v-if="shower.preferences.notes">
              <h5 class="text-sm font-semibold text-gray-700 mb-1">Observações</h5>
              <p class="text-sm text-gray-600">{{ shower.preferences.notes }}</p>
            </div>

            <div v-if="shower.preferences.notAllowedModels">
              <h5 class="text-sm font-semibold text-gray-700 mb-1">Modelos Não Permitidos</h5>
              <p class="text-sm text-gray-600">{{ shower.preferences.notAllowedModels }}</p>
            </div>
          </div>
        </div>
      </UCard>
...
const getTextColor = (colorName: string): string => {
  const darkText = ['Branco', 'Rosê (Rosa Claro)', 'Rosa', 'Pink', 'Lilás', 'Amarelo', 'Nude']
